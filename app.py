...
class App:
  ...
  def setup_ui(self):
    ...
    self.mostrar_informacion.grid(row=6,column=0,columnspan=2)

    self.canva = tk.Canvas(self.root,width=200,height=200)
    self.canva.grid(row=1,column=3,rowspan=6)

  def agregar_laptop(self):
    nueva_laptop = Laptop_gaming(self.marca.get(),self.procesador.get(),self.memoria.get(),self.tarjeta_graf.get(),self.precio.get())
